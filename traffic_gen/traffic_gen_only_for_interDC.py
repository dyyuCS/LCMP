...
if __name__ == "__main__":
  ...
  # 打开并读取 CDF 文件，将其内容存储在 cdf 列表中。
	...
	if dc_count is not None and dc_nnodes is not None:
		host_heap = [(base_t + int(poisson(avg_inter_arrival)), i) for i in range(nhost)]
		...
		while host_heap:
			...
			# 确定 src 所在 DC
			src_dc = src // dc_nnodes
			# 随机选择目标 DC，且不为 src_dc
			other_dcs = list(range(dc_count))
			other_dcs.remove(src_dc)
			dst_dc = random.choice(other_dcs)
			# 在目标 DC 中随机选择主机
			dst = random.randint(dst_dc * dc_nnodes,
													(dst_dc + 1) * dc_nnodes - 1)
