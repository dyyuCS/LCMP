...
public:
  ...
  // [NEW] 添加文件流映射
  static std::map<uint32_t, std::ofstream*> m_flowTraceFiles;

  // [NEW] 添加/删除追踪流的接口
  static void AddTraceFlow(uint32_t flowId, std::string output_dir);
  // [NEW] 资源清理函数,防止内存泄漏
  static void CleanupTraceFiles();
...
private:
  ...
  static std::set<uint32_t> m_traceFlowIds;  // [new]追踪的流ID集合
