...
public:
  ...
  // [new] 添加所属流id,用于追踪指定流的包路由
  void AddTraceFlowId(uint32_t traceFlowId) {
      m_traceFlowId = traceFlowId;
  }
    
  // [new] 获取所属流id,用于追踪指定流的包路由
  uint32_t GetTraceFlowId() const {
      return m_traceFlowId;
  }
...
private:
  ...
  uint32_t m_traceFlowId;  // [new]存储trace flow ID
