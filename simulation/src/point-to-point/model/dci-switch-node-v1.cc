...
if (saveRoutingChoice) {
		...
		for (auto intf_idx : nexthops) {
			...
			uint8_t delay_cost = CalcDelayCost(delay_ms);

			// 2 计算带宽成本
			uint8_t bw_cost = CalcBwCost(m_linkBw[intf_idx]);

			// 3 计算拥塞成本
			uint8_t cong_cost = CalcCongCost(intf_idx, m_linkBw[intf_idx]);

			// 4. 三个元素的加权求和
			const int alpha = 3, beta = 2, gamma = 3;
			const int shift = 3; // 最终右移3位，相当于除以8
			uint32_t total_cost = alpha * delay_cost + beta * bw_cost + gamma * cong_cost;
			total_cost = total_cost >> shift; // 缩放到0~255
...
if (saveRoutingChoice) {
			buffer << "[flow-" << flowId << "][DCI " << this->GetId()
				<< "] port " << intf_idx
				<< ": DelayCost " << (int)delay_cost
				<< ", BwCost " << (int)bw_cost
				<< ", CongCost " << (int)cong_cost
