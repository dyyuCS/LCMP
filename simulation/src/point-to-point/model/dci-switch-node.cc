...
TypeId DCISwitchNode::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::DCISwitchNode")
	...
	.AddAttribute("Mtu",
		...
		MakeUintegerChecker<uint32_t>())
    .AddAttribute ("W_dl", "Weight for delay in static cost calculation.",
                   UintegerValue (3),
                   MakeUintegerAccessor (&DCISwitchNode::m_w_dl),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("W_bw", "Weight for bandwidth in static cost calculation.",
                   UintegerValue (1),
                   MakeUintegerAccessor (&DCISwitchNode::m_w_bw),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("S_static", "Shift factor for static cost normalization.",
                   UintegerValue (2),
                   MakeUintegerAccessor (&DCISwitchNode::m_S_static),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("W_ql", "Weight for queue length in congestion cost calculation.",
                   UintegerValue (2),
                   MakeUintegerAccessor (&DCISwitchNode::m_w_ql),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("W_tl", "Weight for trend level in congestion cost calculation.",
                   UintegerValue (1),
                   MakeUintegerAccessor (&DCISwitchNode::m_w_tl),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("W_dp", "Weight for duration penalty in congestion cost calculation.",
                   UintegerValue (1),
                   MakeUintegerAccessor (&DCISwitchNode::m_w_dp),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("S_cong", "Shift factor for congestion cost normalization.",
                   UintegerValue (3),
                   MakeUintegerAccessor (&DCISwitchNode::m_S_cong),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Alpha", "Weight for static cost in total cost calculation.",
                   UintegerValue (3),
                   MakeUintegerAccessor (&DCISwitchNode::m_alpha),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Beta", "Weight for congestion cost in total cost calculation.",
                   UintegerValue (1),
                   MakeUintegerAccessor (&DCISwitchNode::m_beta),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("S_total", "Shift factor for total cost normalization.",
                   UintegerValue (2),
                   MakeUintegerAccessor (&DCISwitchNode::m_S_total),
                   MakeUintegerChecker<uint32_t> ())
  ;
  return tid;
}
...
int DCISwitchNode::GetOutDev(Ptr<const Packet> p, CustomHeader &ch){
	...
	if (m_routingMode == 2) { // 0: ECMP, 1: UCMP, 2: Ours
		...
		for (auto intf_idx : nexthops) {
			...
			// std::cout << "DelayCost: " <<  delay_score<< std::endl;
            uint8_t bw_score = CalcBwCost(m_linkBw[intf_idx]); // 计算链路容量成本
			// std::cout << "BWcost:"  << bw_score << std::endl;
            uint32_t staticScore = m_w_dl * delay_score + m_w_bw * bw_score;
			// std::cout << "StaticCost: " << staticScore << std::endl;
            uint8_t C_static = std::min(staticScore >> m_S_static, 255u);


			// 2 计算拥塞成本
			MonitorCongestionState(); // 先更新当前队列拥塞状态
			...
			uint8_t TrendLevel = CalcTrendLevel(intf_idx, m_linkBw[intf_idx]);

			uint32_t congScore = m_w_ql * QLevel + m_w_tl * TrendLevel + m_w_dp * DurationPenalty;
			uint8_t C_cong = std::min(congScore >> m_S_cong, 255u);

			// 3 三个元素加权求和
            uint32_t total_cost_score = m_alpha * C_static + m_beta * C_cong ;
            uint8_t C_cost = std::min(total_cost_score >> m_S_total, 255u);

			cost_path_pairs.emplace_back(C_cost, intf_idx);


		}


		const uint8_t COST_THRESHOLD = static_cast<uint8_t>(255 * 0.8); // 80%阈值
		// const uint8_t COST_THRESHOLD = static_cast<uint8_t>(255 * 0.5); // 50%阈值
		// 排序并选取前一半
		...
		if( half < 2) half = cost_path_pairs.size(); // 至少保留2个端口
		for (size_t i = 0; i < half; i++) {
			if (cost_path_pairs[i].first < COST_THRESHOLD) { // 增加一个阈值筛选，避免极端高拥塞路径被选入 available_paths
		...
		if (!available_paths.empty()) {
			...
			uint32_t idx = EcmpHash(buf.u8, 12, m_ecmpSeed) % cost_path_pairs.size();
            selected_intf = cost_path_pairs[idx].second;
...
{
    // 最大参考时延=MAX_DELAY (512ms)，权重系数=255，右移MAX_DELAY_SHIFT位（÷512）
	uint64_t max_delay = 1 << MAX_DELAY_SHIFT;
	// std::cout << "[TEST] CalcDelayCost: [DCI " << GetId() << "] one_way_delay_ms=" << one_way_delay_ms << " max_delay=" << max_delay << std::endl;
	uint8_t cost = (one_way_delay_ms * 255) >> MAX_DELAY_SHIFT;
    if (one_way_delay_ms > max_delay)
        return 255; // 超过最大参考时延，返回最大成本
    else
		// std::cout << "DelayCost: " << cost << std::endl;
        return cost;
}
...
uint8_t DCISwitchNode::CalcBwCost(uint64_t bw_bps)
{
	...
	// std::cout << "[TEST] CalcBwCost: [DCI " << GetId() << "] " << " bw_gbps=" << bw_gbps << std::endl;
    for (int i = kClassNum - 1; i >= 0; i--) {
        if (bw_gbps >= m_bw_thresh[i]){
		// std::cout << "BWcost:"  << 255 - levelScore[i] << std::endl;
